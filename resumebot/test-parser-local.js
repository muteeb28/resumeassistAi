import { parseResumeToStructuredJSON } from './server/services/advancedResumeParser.js';
import { parseResumeWithStructure } from './server/services/enhancedResumeParser.js';
import fs from 'fs';
...
async function testParsers() {
    ...
    try {
        const advancedResult = parseResumeToStructuredJSON(sampleText);
        console.log('Advanced Parser Sections:', Object.keys(advancedResult).filter(k => !k.startsWith('_')));
        console.log('Summary length:', advancedResult.summary?.length || 0);
        console.log('Experience entries:', advancedResult.experience?.length || 0);
        console.log('Skills count:', advancedResult.skills?.length || 0);

        fs.writeFileSync('advanced_output.json', JSON.stringify(advancedResult, null, 2));
        console.log('Advanced output saved to advanced_output.json');
    } catch (error) {
    ...
    try {
        const enhancedResult = parseResumeWithStructure(sampleText, { format: 'text', originalPageCount: 1 });
        console.log('Enhanced Parser Sections:', Object.keys(enhancedResult).filter(k => k !== 'structure' && k !== 'formatInfo'));
        console.log('Summary length:', enhancedResult.summary?.length || 0);
        console.log('Experience entries:', enhancedResult.experience?.length || 0);
        console.log('Skills count:', enhancedResult.skills?.length || 0);

        fs.writeFileSync('enhanced_output.json', JSON.stringify(enhancedResult, null, 2));
        console.log('Enhanced output saved to enhanced_output.json');
    } catch (error) {
